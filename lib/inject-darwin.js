...
// Cheap one-shot check for the display-only ("vetrina") mode. osascript ships
// with macOS, so this is nearly always available — the Accessibility grant
// can't be probed without actually sending a keystroke, so permission failures
// surface at click time through the CHECK-ENGINE telltale instead.
async function probe() {
  try {
    await runOsascript(['-e', 'return 1']);
